...
const POOL_SIZE = 1000; // Maximum number of concurrent connections allowed.
const MAX_DOCS = 1000; // The maximum number of docs to send in a single batch

function noOp(err) { if (err) throw err; }

var pool = new Array(POOL_SIZE);
var poolIndex = 0;
function getServerFromPool(host, port, callback) {
...
function CouchClient(url) {
  ...
  function request(method, path, body, callback) {
    ...
    getServerFromPool(uri.host, function (err, server) {
    ...
    function consumeQueue() {
      ...
      request("POST", path, {keys: keys}, function (err, results) {
        ...
        results.rows.forEach(function (result, i) {
          if (includeDoc) {
            if (result.error) {
              var err = new Error("CouchDB Error: " + JSON.stringify(result));
